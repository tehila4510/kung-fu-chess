...
async def test_command_before_auth():
    print("=== 1. Command before AUTH is rejected ===")
    async with websockets.connect(SERVER) as ws:
        assert (await recv_json(ws))["type"] == "auth_required"
        await ws.send("WMe2e4")
        err = await recv_json(ws)
        assert err == {"type": "error", "reason": "auth_required"}, err
    print("OK: unauthenticated command -> error/auth_required")


async def test_auth_no_auto_seat():
    print("\n=== 2. AUTH does not auto-seat (no welcome/state) ===")
    user = uniq("auth_only")
    async with websockets.connect(SERVER) as ws:
        assert (await recv_json(ws))["type"] == "auth_required"
...
async def test_play_searching_alone():
    print("\n=== 3. AUTH then PLAY alone -> searching (still no seat) ===")
    user = uniq("search")
...
async def test_double_play_while_searching():
    print("\n=== 4a. Double PLAY while searching ===")
    user = uniq("dbl_q")
    ws, auth = await connect_and_auth(user)
    try:
        assert auth["type"] == "auth_ok" and auth.get("role") == "player", auth
        ...
        assert first["type"] == "searching", first

        await ws.send("PLAY")
        second = await recv_json(ws)
        assert second == {"type": "error", "reason": "already_searching"}, second

        # Still only one queue entry: no second searching / no welcome alone
        try:
            extra = await asyncio.wait_for(ws.recv(), timeout=0.5)
            raise AssertionError(f"unexpected message after double PLAY: {extra}")
        except asyncio.TimeoutError:
            pass
        print("OK: second PLAY -> already_searching, no re-queue")
    finally:
        await ws.close()


async def test_double_play_while_seated():
    print("\n=== 4b. PLAY again while already seated ===")
    u1, u2 = uniq("seat_a"), uniq("seat_b")
    ...
    try:
        ...
        await ws1.send("PLAY")
        assert (await recv_json(ws1))["type"] == "searching"
        await ws2.send("PLAY")
        ...
        await ws1.send("PLAY")
        # Seated PLAY is not re-queued; handled as a game wire command.
        rejected = await recv_json(ws1)
        assert rejected["type"] == "state", rejected
        assert rejected["status"] == "rejected", rejected
        assert rejected["reason"] == "unknown_command", rejected

        # Opponent also gets the room broadcast; no new welcome/match.
        peer = await recv_json(ws2)
        assert peer["type"] == "state", peer
        assert peer["status"] == "rejected", peer
        assert peer["reason"] == "unknown_command", peer

        try:
            extra = await asyncio.wait_for(ws1.recv(), timeout=0.5)
            raise AssertionError(f"unexpected extra after seated PLAY: {extra}")
        except asyncio.TimeoutError:
            pass
        print("OK: seated PLAY -> rejected/unknown_command, no duplicate match")
    finally:
        await ws1.close()
        await ws2.close()
...
async def test_match_two_players():
    print("\n=== 5. Two close-ELO clients match and get W/B seats ===")
    u1, u2 = uniq("w"), uniq("b")
    ws1, auth1 = await connect_and_auth(u1)
    ws2, auth2 = await connect_and_auth(u2)
    try:
...
async def test_viewer_third_client(ws1, ws2):
    print("\n=== 6. Third client -> viewer (not rejected, no seat) ===")
    user = uniq("viewer")
...
async def test_double_login():
    print("\n=== 7. Same username from two devices is rejected ===")
    user = uniq("double")
    ws1, auth1 = await connect_and_auth(user)
    try:
        assert auth1["type"] == "auth_ok", auth1
        ws2 = await websockets.connect(SERVER)
        try:
            assert (await recv_json(ws2))["type"] == "auth_required"
            await ws2.send(f"AUTH {user} 1234")
            err = await recv_json(ws2)
            assert err == {"type": "error", "reason": "already_logged_in"}, err
            print("OK: second device -> already_logged_in")
        finally:
            await ws2.close()
    finally:
        await ws1.close()
...
async def test_wrong_password():
    print("\n=== 8. Wrong password still rejected ===")
    user = uniq("pwd")
    ws, auth = await connect_and_auth(user)
    await ws.close()
...
async def test_match_timeout():
    print("\n=== 9. PLAY alone -> match_not_found after ~60s ===")
    user = uniq("timeout")
    ws, auth = await connect_and_auth(user)
    try:
        assert auth["type"] == "auth_ok" and auth.get("role") == "player", auth
